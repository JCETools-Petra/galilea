...
class Home extends CI_Controller {
    ...
    public function index() {
        ...
        // 1. GET SLIDER DATA (NEW)
        $slider_json = get_frontend_settings('slider_images');
        $page_data['slider_images'] = json_decode($slider_json);

        // 2. PENGUMUMAN TERBARU
        $this->db->limit(3);
        $this->db->order_by('id', 'desc'); 
        $page_data['frontend_notices'] = $this->db->get_where('noticeboard', array('show_on_website' => 1))->result_array();
        
        // 3. DATA DOSEN LENGKAP
        $this->db->select('u.id, u.name, u.email, t.designation, t.social_links');
        $this->db->from('users u');
        $this->db->join('teachers t', 'u.id = t.user_id');
        $this->db->where('u.role', 'teacher');
        $this->db->limit(10);
        $page_data['lecturers'] = $this->db->get()->result_array();

        // 4. DATA ACARA (EVENTS)
        $page_data['upcoming_events'] = $this->frontend_model->get_frontend_upcoming_events();

        // 5. STATISTIK
        $page_data['stat_students'] = $this->db->where('role', 'student')->count_all_results('users');
        $page_data['stat_teachers'] = $this->db->where('role', 'teacher')->count_all_results('users');
        $page_data['stat_parents']  = $this->db->where('role', 'parent')->count_all_results('users');
